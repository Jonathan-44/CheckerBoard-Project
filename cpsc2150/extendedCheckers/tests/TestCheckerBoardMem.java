    ...
    private String arrayToString(ICheckerBoard board, char[][] array) {
        ...
        for (int row = 0; row < array[1].length; row++) {
            ...
            for (int col = 0; col < array[0].length; col++) {
                ...
                char piece = board.whatsAtPos(pos);
                if (piece == ' ') {
                    if ((row + col) % 2 != 0) {
                        sb.append("|* ");
                    } else {
        ...
        public void testInitialDirectionAddition(){
            ...
            board.addViableDirections(player, DirectionEnum.NE);
            board.addViableDirections(player, DirectionEnum.NW);

            ArrayList<DirectionEnum> expected = new ArrayList<>();
            expected.add(DirectionEnum.SW);
            expected.add(DirectionEnum.SE);

            ArrayList<DirectionEnum> observed = board.getViableDirections().get(player);
        ...
        public void testCrownAlreadyCrownedPiece() {
            ...
            board.placePiece(startPos, player);
            //Crown a player one piece
            board.movePiece(startPos, DirectionEnum.SE);
            //Move crowned piece away from edge
            board.movePiece(endPos, DirectionEnum.NW);
            //Move crowned piece back to edge
            board.movePiece(startPos, DirectionEnum.SE);
